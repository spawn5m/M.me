...
import { writeMaintenancePreviewEnabled } from '../../../lib/maintenance-preview'
...
beforeEach(() => {
  ...
  it('torna a mostrare la manutenzione quando la preview viene disattivata', async () => {
    window.sessionStorage.setItem('admin-maintenance-preview-enabled', 'true')
    mockUseAuth.mockReturnValue({
      user: { id: '1' },
      permissions: ['maintenance.manage'],
    ...
    act(() => {
      writeMaintenancePreviewEnabled(false)
    })
