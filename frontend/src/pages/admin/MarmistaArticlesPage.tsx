  ...
  return (
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl text-[#1A2B4A]" style={{ fontFamily: 'Playfair Display, serif' }}>Articoli Marmisti</h1>
      ...
      {tab === 'import' && (
        <div className="max-w-xl">
          <p className="text-sm text-[#6B7280] mb-4">Carica un file Excel con colonne: <code className="bg-[#F8F7F4] px-1 rounded">codice, descrizione, note, prezzo_pubblico, categorie</code></p>
      ...
      <FormModal isOpen={isCreating || !!editing} title={editing ? 'Modifica Articolo' : 'Nuovo Articolo'} onClose={closeModal} onSubmit={onSubmit} isSubmitting={isSubmitting}>
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-[#1A1A1A] mb-1">Prezzo Pubblico (€)</label>
              <input type="number" step="0.01" {...register('publicPrice')} className="w-full border border-[#E5E0D8] rounded px-3 py-2 text-sm focus:outline-none focus:border-[#1A2B4A]" />
            </div>
            <div>
